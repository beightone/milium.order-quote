...
const routes = {
  ...
  listCarts: (account: string, email: string) =>
    `${routes.cartEntity(
      account
    )}/search?email=${email}&_schema=${SCHEMA_VERSION}&_sort=creationDate DESC`,
  // Esta rota deve ser usada para obter um documento específico pelo ID
  getCart: (account: string, id: string) =>
    `${routes.cartEntity(account)}/documents/${id}?_schema=${SCHEMA_VERSION}`,
